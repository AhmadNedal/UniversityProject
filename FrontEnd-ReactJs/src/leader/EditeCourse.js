...
import { TimeList ,major } from '../Shared';
import LeaderHeader from '../AllHeader/LeaderHeader';
import CheackLeader from './CheckLeader';

const EditCourseForm = () => {
  const location = useLocation();
  const { SendCourse } = location.state || {};
  const initialCourse= {...SendCourse}
  const navigate = useNavigate();  

  const [form, setForm] = useState(initialCourse);
  const [teacher,setTeacher] = useState([{name:"sdf" , id:332},{name:"sdf" , id:332}]); 
